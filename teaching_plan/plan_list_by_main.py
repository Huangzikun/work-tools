...
file_dir = "/Users/huangzikun/Desktop/数据结构实验大纲.docx"
teaching_plan_count = 12
step = 4
res_list = []
    ...
    completion = client.chat.completions.create(
       # 指定您创建的方舟推理接入点 ID，此处已帮您修改为您的推理接入点 ID
        ...
        messages=[
            ...
            {"role": "user", "content": f"根据文档内容中的教学时数/2确定教案的数量，如教学时数为10，即你应该给我提供10/5=5个教案的内容。总共应提供{teaching_plan_count}个教案。当前输出第{i}~{i+step-1}个教案。列表内容如下：\n{type}\n。文档内容如下：\n{doc_content}\n如果文档未提及，则对象内的这一项设置为空字符串。如果一项中包含一个列表，则每一个条目后面增加换行。"},
        ],
        response_format={
            "type": "json_object",
        },
...
with open("/Users/huangzikun/PycharmProjects/work-tools/teaching_plan/temp_plan.txt", "w", encoding="utf-8") as f:
